var sketchProc = function (processingInstance) {
  with (processingInstance) {
    ...
    var Dragon = function () {
      ...
      this.setup = function () {
        for (var i = 0; i <= 27; i++) {
          this.segments.push({ x: this.x + (this.w * i) / 27, y: this.y });
        }
      };
      this.setup();
      this.update = function () {
        ...
        for (var i = 0; i < this.segments.length; i++) {
          ...
          if (i === 0) {
            ...
            segment.y = lerp(segment.y, this.segments[i - 1].y, 0.2);
            segment.x = lerp(segment.x, this.segments[i - 1].x, 0.2);
          }
        }
      ...
      this.drawRightWing = function () {
        ...
        bezierVertex(
          this.wing.spines.b.x2 - 10,
          this.wing.spines.b.y2,
          this.wing.spines.c.x2 - 15,
        ...
        bezierVertex(
          this.wing.spines.c.x2 - 10,
          this.wing.spines.c.y2,
          this.wing.spines.d.x2 - 15,
        ...
        bezier(
          ...
          this.wing.y3,
          this.wing.x3 - 3,
          this.wing.y3 + (this.wing.y4 - this.wing.y3) * 0.3,
          this.wing.x4 - 2,
          this.wing.y4 - (this.wing.y4 - this.wing.y3) * 0.5,
          this.wing.x4,
          this.wing.y4
        );
        ...
        triangle(
          ...
          this.wing.y4,
          this.wing.x4,
          this.wing.y4 + (this.wing.y4 - this.wing.y3)
        );
        ...
        line(
          this.wing.x3,
          this.wing.y3,
          this.wing.spines.b.x1,
          this.wing.spines.b.y1
        );
        line(
          this.wing.spines.b.x1,
          this.wing.spines.b.y1,
        ...
        line(
          this.wing.x3,
          this.wing.y3,
          this.wing.spines.c.x1,
          this.wing.spines.c.y1
        );
        line(
          this.wing.spines.c.x1,
          this.wing.spines.c.y1,
      ...
      this.draw = function () {
        ...
        for (var i = 2; i < this.segments.length - 5; i++) {
          ...
          triangle(
            this.segments[i - 1].x,
            this.segments[i - 1].y + this.nodes[i] * 0.1,
            segment.x,
            segment.y + this.nodes[i] * 0.1,
            segment.x - 8 + cos(a) * 12,
        ...
        beginShape();
        vertex(
          this.segments[this.segments.length - 2].x - this.diff * 0.1,
          this.segments[this.segments.length - 2].y - 5
        );
        vertex(
        ...
        bezierVertex(
          this.segments[this.segments.length - 2].x + this.diff * 0.1 + 3,
          this.segments[this.segments.length - 2].y + 2,
          this.segments[this.segments.length - 2].x - this.diff * 0.1 + 3,
          this.segments[this.segments.length - 2].y - 2,
          this.segments[this.segments.length - 2].x - this.diff * 0.1,
          this.segments[this.segments.length - 2].y - 5
    ...
    var Scene = function () {
      ...
      this.cloudsBack = [];
      this.cloudsFront = [];
      this.grounds = [{ x: 0 }, { x: -width }];
      this.hillsFront = [{ x: 0 }, { x: -599 }];
      this.hillsBack = [{ x: 0 }, { x: -599 }];
      this.theme = 0;
      ...
      this.setup = function () {
        this.images = {
          backTan: function () {
            ...
            for (var i = 0; i <= 450; i++) {
              fill(
                lerpColor(color(135, 206, 235), color(135, 206, 235), i / 450)
              );
              rect(0, i, width, i);
            }
        ...
        // Generar imágenes inmediatamente
        var keys = Object.keys(this.images);
        for (var i = 0; i < keys.length; i++) {
          this.images[keys[i]] = this.images[keys[i]]();
        }
        ...
        for (var i = 0; i < 15; i++) {
          ...
          var cloudImage = get(50, 50, 110, 110);
      ...
      this.runClouds = function (clouds) {
        for (var i = 0; i < clouds.length; i++) {
          ...
          image(
            ...
            cloud.x,
            cloud.y,
            cloud.img.width * cloud.scale,
            cloud.img.height * cloud.scale * 0.8
          );
          cloud.x += cloud.speed;
          if (cloud.x > width) {
            cloud.x = -200;
      ...
      this.draw = function () {
        ...
        for (var i = 0; i < this.hillsBack.length; i++) {
          image(
            ...
            this.hillsBack[i].x,
            305,
            this.images.hillsBack.width + 14,
            this.images.hillsBack.height
          );
          this.hillsBack[i].x += this.speed * 1.1;
          if (this.hillsBack[i].x >= width) this.hillsBack[i].x = -600;
        }
        ...
        for (var i = 0; i < this.hillsFront.length; i++) {
          image(
            ...
            this.hillsFront[i].x,
            385,
            this.images.hillsFront.width + 2,
            this.images.hillsFront.height
          );
          this.hillsFront[i].x += this.speed * 1.2;
          if (this.hillsFront[i].x >= width) this.hillsFront[i].x = -600;
        }
        ...
        for (var i = 0; i < this.grounds.length; i++) {
          image(
            ...
            this.grounds[i].x,
            450,
            this.groundImage.width + 2,
            this.groundImage.height
          );
          this.grounds[i].x += this.speed * 1.5;
          if (this.grounds[i].x >= width) this.grounds[i].x = -width;
        }
      };

      this.go = function () {
        this.draw();
        if (this.showClouds) this.runClouds(this.cloudsBack);
        this.dragon.fly();
        if (this.showClouds) this.runClouds(this.cloudsFront);
      };
    };
